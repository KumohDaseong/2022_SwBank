...
Fixme > 제품의 표기된 세부정보가 각 제품마다 조금씩 차이가 있습니다. 분석이 필요합니다
...
for index in range(max_page):
    ...
    for selected_product in all_products_in_page:
        ...
        current_url = product_detail_url
        driver.get(current_url)
        
        product_name = driver.find_element_by_id("productTitle")
        product_price = driver.find_elements_by_class_name("a-offscreen")[0]
        product_review_rating = driver.find_elements_by_class_name("a-icon-alt")[4]
        product_standard = driver.find_elements_by_class_name("a-size-base.prodDetAttrValue")[0]
        product_date_first_available = driver.find_elements_by_class_name("a-size-base.prodDetAttrValue")[6]
        product_manufacturer = driver.find_elements_by_class_name("a-size-base.prodDetAttrValue")[2]
        product_asin = driver.find_elements_by_class_name("a-size-base.prodDetAttrValue")[3]
        ...
        print(product_standard)
        print(product_date_first_available)
        print(product_manufacturer)
        print(product_asin,"    ",i)
        print()
        print()
        print()
